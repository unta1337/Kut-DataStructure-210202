...
// 새로 추가한 insert()를 테스트하기 위함.
TEST(UnsortedArrayList_Duplicate_OrderPreserving, insertTest){
	UnsortedArrayList list{1,2,3,5,6,7,8};
	list.insert(0, 0);
	list.insert(4, 4);
	ASSERT_THROW(list.insert(-1, 10), std::out_of_range);
	ASSERT_THROW(list.insert(10, 10), std::out_of_range);
	list.insert(9, 9);
	ASSERT_THROW(list.insert(4, 10), std::runtime_error);
	std::string output = "";
	while(!list.isEmpty())
		output += std::to_string(list.popFront())+",";
